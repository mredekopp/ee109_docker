...
function docker_run_command() {
  docker run -v "${work}":/work -d -t --security-opt seccomp:unconfined --cap-add SYS_PTRACE ee109 1> "${container}"
}
...
function docker_admin_run_command() {
  docker run -v "${work}":/work -d -t --security-opt seccomp:unconfined --cap-add SYS_ADMIN ee109 1> "${container}"
}
